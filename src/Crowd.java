...
{
    public class Boid {
        double posx, posy, velx, vely, frcx, frcy, direction;
        long lastTime;
        public void update() {
        ...
        public void draw(GLAutoDrawable gld) {
            ...
            // set color
            double color = (velx * velx + vely * vely) / 1.0;
            ...
            gl.glVertex3d(posx-size, posy+size, 0);
            gl.glVertex3d(posx+size, posy, 0);
       //     gl.glRotatef((float) Math.toDegrees(direction), 0, 0, 1);
       //     gl.glVertex3d(posx-size, posy+size, 0);
            gl.glEnd();
        }
    ...
    public synchronized void update() {
        ...
        while (bi.hasNext()) {
            ...
            // loop over all boids
            ListIterator<Boid> bj = boids.listIterator(); 
            while (bj.hasNext()) {
                Boid j = bj.next();
    ...
    public synchronized void display (GLAutoDrawable gld)
    {
      ...
      //  glu.gluPerspective(0.0, winWidth, 0.0, winHeight);
